#!/usr/bin/env bash
set -euo pipefail
for f in docs/release/release_process.md docs/release/versioning.md docs/release/compat_matrix.md docs/release/downgrade.md docs/runbooks/release_signing_failed.md RELEASE_CHECKLIST.md CHANGELOG.md docs/ops/go_no_go_template.md docs/en/ops/go_no_go_template.md docs/governance/release_decisions/latest_go_no_go.md; do
  test -s "$f"
done
