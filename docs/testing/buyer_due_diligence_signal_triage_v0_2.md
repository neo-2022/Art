...
| Кодовая база монолитна в ключевых entrypoint-файлах | Подтверждено частично: `core/src/main.rs=8204` строк и остаётся монолитным runtime-entrypoint; при этом внешние цифры по `agent/main.rs`, `console-web/main.ts`, `local-stores/index.ts`, `browser/outbox.js` уже устарели (`821`, `1877`, `1029`, `470`) и больше не подтверждают заявленный масштаб проблемы | `PARTIAL` | Даёт нам точный инженерный вывод: риск code concentration реально живёт прежде всего в `core`, а не одинаково во всех перечисленных файлах; decomposition должна идти точечно, а не по устаревшей картине | 04, 11, 12, 17, 18, 28, 35, 37, 39 |
...
| Тестовая культура есть, но console/browser не везде закрыты глубокими hostile e2e-сценариями | Подтверждено: в `apps/console-web/test/console-web.test.mjs` 91 строковая/render-проверка при нулевом e2e/runtime слое; в `browser/test/*` уже есть сильные chaos/outbox tests, но console/browser corpus в целом не доказывает длинные rollback/migration/incident chains | `CONFIRMED` | Даёт нам отдельный remediation-трек: не отрицать наличие тестов, а усилить именно глубину hostile integration/e2e proof для UI/runtime слоёв | 10, 16, 22, 24, 28, 34, 36, 38 |
