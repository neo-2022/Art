...
| `23` | `OPEN` | Downstream DR/backup contour для `DEF-001` уже стал реальным runtime-proof: `WAL-aware backup`, `DR drill`, runtime smoke, PR CI path и `observability_gap.tls_config_invalid` как persisted startup backlog materialized. Единственный оставшийся blocker stage23 — честный TLS hot-reload без простоя по `SIGHUP`. | `scripts/tests/ops_stage23_smoke.sh`, `scripts/tests/tls_config_invalid_runtime.sh`, `docs/ops/backup_restore.md`, `docs/ops/dr_drill.md`, `docs/ops/tls_rotation.md`, `scripts/ci/check_ops_stage23_docs.sh`, `docs/governance/evidence/stage23_ops_runtime_smoke.log`, `docs/governance/evidence/stage23_tls_config_invalid.log` | оставить открытым |
