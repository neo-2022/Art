...
| `11` | `OPEN` | Systemd vacuum path уже исправлен, а живой `art-core` теперь уже пишет и reload-ит `v1/v2` события через SQLite; корневой blocker сместился глубже: durable basement покрывает только событийные потоки, но не `incidents`, `audits`, derived-state и полный storage contour | `core/src/main.rs`, `docs/core/storage.md`, `scripts/storage_stage11.py`, `docs/governance/evidence/stage11_core_sqlite_restart.log` | оставить открытым |
