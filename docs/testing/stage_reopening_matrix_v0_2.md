...
| `11` | `OPEN` | Systemd vacuum path уже исправлен, а живой `art-core` теперь уже пишет и reload-ит через SQLite `v1/v2` события, `incidents` и `audit chain`; корневой blocker сместился глубже: durable basement всё ещё не покрывает DNA-derived state, fingerprint/source indexes, часть аналитики и полный recovery contour | `core/src/main.rs`, `docs/core/storage.md`, `scripts/storage_stage11.py`, `docs/governance/evidence/stage11_core_sqlite_restart.log` | оставить открытым |
