...
Отдельный защитный слой навигации и синхронизации для этой программы задаётся документом
`docs/portal/DOCUMENTATION_TREE.md`. Он не управляет corrective-order напрямую, но показывает,
какие документы связаны с корнем проекта, сколько строк содержит каждый узел, какие каталоговые
узлы участвуют в кроне, и где изменение документа уже требует синхронной правки зависимого контура.
...
`DOCUMENTATION_TREE.md` здесь нужен не как "красивый обзор", а как защитный механизм от
документационного дрейфа:
- он строится от единственного корня `README.md`;
- считает суммарные строки по дереву и по каталоговым узлам;
- помогает понять, какие документы могут быть затронуты при изменении корня или ствола;
- подаёт сигнал, если дерево не синхронизировано с реальными файлами.
...
- подключённая внешняя система становится видимой оператору как отдельная сущность со статусом, типами данных и active gaps, а не скрытым конфигом;
