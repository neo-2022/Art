...
### Уровень 2 — Дефектовочная ведомость
Этот документ — мост между аудитом и `MASTER`.
...
### Уровень 3 — MASTER
`MASTER` не является самостоятельным источником порядка remediation.
...
### Уровень 4 — Stage checklist execution
Чек-листы остаются исполнительным механизмом.

Но теперь они работают так:
- stage берётся из `MASTER`;
- `MASTER` берёт порядок из этой ведомости;
- эта ведомость берёт порядок из аудита и канона проекта.

Итоговая цепочка:

`Корневые документы -> Аудит -> Дефектовочная ведомость -> MASTER -> Stage checklist -> Код/тесты/документы/runtime`
