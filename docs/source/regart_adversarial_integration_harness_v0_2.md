...
Документ обязан быть человекочитаемым: инженер, оператор и reviewer должны без чтения исходного кода понимать, зачем harness нужен, какие сценарии он покрывает и почему он не подменяет базовые тесты самого `Art`.
