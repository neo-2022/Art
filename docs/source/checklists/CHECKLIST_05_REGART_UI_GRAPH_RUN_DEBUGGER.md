...
## CI gate
- [x] В CI workflow включён запуск тестов из раздела “Тестирование” через `scripts/ci/check_stage05_wrapper.sh` в двух режимах:
  - [x] default CI mode (`STAGE05_EXTERNAL_STRICT=0`) не зависит от внешнего репозитория и запускает локальные runtime smoke: `npm --prefix browser test -- test/multitab.e2e.test.js test/outbox.compression.test.js`
  - [x] strict mode (`STAGE05_EXTERNAL_STRICT=1`) использует локальный checkout `my_langgraph_agent` и запускает source-of-truth тесты `tests/multiTabManager.spec.js tests/outbox.spec.js tests/uiProxyGap.spec.js`
