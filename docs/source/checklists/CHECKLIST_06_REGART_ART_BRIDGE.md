...
- [ ] **1. Сделать:** Внести в `CHECKLIST_REGART_ART_INTEGRATION.md` политику overflow для Outbox/Spool и реализовать её (см. новый test + `scripts/ci/check_stage06_wrapper.sh`).
  - [ ] Политика по умолчанию: `never_drop_unacked`
    - [ ] при заполнении: reject new (см. `test_never_drop_unacked_rejects_new` + `scripts/ci/check_stage06_wrapper.sh` вывод `observability_gap.spool_full`).
    - [ ] генерируется `observability_gap.outbox_full` и/или `observability_gap.spool_full` (логируется `observability_gap.spool_full` + 507 `spool_full`).
  - [ ] Альтернативный режим: `drop_oldest_when_full` (подтвержден `scripts/ci/check_stage06_wrapper.sh` + `test_drop_oldest_when_full_logs_lossy`)
  ...
  - [ ] **Проверка (pass/fail):** в `CHECKLIST_REGART_ART_INTEGRATION.md` добавлены пункты про оба режима + тесты, log `lossy_mode_active`, `data_quality.lossy_*` зафиксирован.

- [ ] **2. Сделать:** Внести требование и реализовать: управление сервисами выполняется только через Art Actions `POST /api/v1/actions/execute`. Evidence: `tests/integration_tests/test_ui_proxy_service_actions.py` + `scripts/ci/check_stage06_wrapper.sh`.
  - [ ] прямые вызовы `systemctl`, `tmux`, shell-скриптов из UI Proxy запрещены (`rg "systemctl|tmux" agent/src/react_agent/ui_proxy.py` = 0).
  - [ ] UI Proxy реализует только Art Actions API (`service_control`, `service_status`) и при сбоях логирует `observability_gap.actions.failure`.
  - [ ] **Проверка (pass/fail):** `tests/integration_tests/test_ui_proxy_service_actions.py` + `scripts/ci/check_stage06_wrapper.sh`.

- [ ] **3. Сделать:** Внести требование и реализовать: UI Proxy↔Art Core всегда HTTPS.
  - [ ] для dev допускается self-signed сертификат (`ART_TLS_VERIFY=0`)
  - [ ] HTTP запрещён (runtime блокирует non-https URL в `_ensure_art_tls_config`)
  - [ ] **Проверка (pass/fail):** в `CHECKLIST_REGART_ART_INTEGRATION.md` пункт “HTTPS-only” закрыт `[x]`; evidence: `test_art_ingest_https_only_rejects_http`, `test_art_ingest_tls_smoke_self_signed`.
...
- [ ] **4. Сделать:** Внести требование и реализовать: upstream ошибки оформляются единым форматом и порождают RawEvent `kind="upstream_error"`.
  ...
  - [ ] **Проверка (pass/fail):** в `CHECKLIST_REGART_ART_INTEGRATION.md` есть пункт “upstream_error RawEvent format + test”, закрыт `[x]`; evidence: `test_upstream_error_format_contains_required_fields`.
...
- [ ] **5. Сделать:** Внести требование и реализовать: RawEvent содержит `retry_count >= 0` (текущий хоп).
  ...
  - [ ] **Проверка (pass/fail):** в `CHECKLIST_REGART_ART_INTEGRATION.md` есть пункт “retry_count contract + test”, закрыт `[x]`; evidence: `test_retry_count_present_and_non_negative`.
...
- [ ] **6. Сделать:** Внести требование и реализовать: audit immutability test (append-only) обязателен.
  ...
  - [ ] **Проверка (pass/fail):** в `CHECKLIST_REGART_ART_INTEGRATION.md` есть пункт “audit immutability test”, закрыт `[x]`; evidence: `test_audit_immutability_append_only`.
...
- [ ] integration: путь управления через Art Actions `POST /api/v1/actions/execute` (`tests/integration_tests/test_ui_proxy_service_actions.py`)
...
- [ ] В CI workflow включён запуск тестов из раздела “Тестирование” через `scripts/ci/check_stage06_wrapper.sh` в двух режимах:
  - [ ] default CI mode (`STAGE06_EXTERNAL_STRICT=0`) не зависит от внешнего репозитория и проверяет локальный wrapper + runbook/docs + непротиворечивость чек-листа
  - [ ] strict mode (`STAGE06_EXTERNAL_STRICT=1`) использует локальный checkout `my_langgraph_agent` и выполняет кодовые проверки source-of-truth (`ui_proxy.py` + integration tests patterns + AST checks)
...
- [ ] `scripts/ci/check_stage06_wrapper.sh` проходил: `./scripts/ci/check_stage06_wrapper.sh` (7 passed).  
- [ ] `agent/tests/integration_tests/test_ui_art_ingest.py::test_drop_oldest_when_full_logs_lossy` подтверждает `drop_oldest_when_full`, `lossy_mode_active` и `data_quality.lossy`.
