...
- [x] **2. Сделать:** В CI включить Rust fmt/clippy/test и Browser lint/test/build.
  ...
  - [x] Browser: test
  - [x] Browser: build
  - [x] **Проверка (pass/fail):** существуют workflow-файлы CI, jobs запускаются на PR в main и зелёные.
...
- [x] **3. Сделать:** В CI включить `gitleaks` (secrets scan) как blocking job.
  ...
  - [x] **Проверка (pass/fail):** job существует, и при искусственном добавлении тестового секрета (в отдельной ветке) job падает.
...
- [x] **4. Сделать:** В CI включить license checks: `cargo-deny` (licenses) и JS license checker (инструмент фиксирован).
  ...
  - [x] **Проверка (pass/fail):** jobs существуют и запускаются на PR в main.
...
- [x] **5. Сделать:** В README закрепить контракты как источник правды (декларация Stage 07) и указать, что сами файлы контрактов добавляются на Stage 08.
  ...
  - [x] **Проверка (pass/fail):** README содержит раздел `Contracts` со всеми пунктами выше.
...
- [x] **6. Сделать:** Создать RU dev docs: как собрать/запустить/прогнать тесты.
  ...
  - [x] **Проверка (pass/fail):** документ существует и содержит команды, которые реально запускаются.
...
- [x] **8. Сделать:** Добавить CI gate Stage 07 для минимальной валидации содержимого (не только существование файлов).
  ...
  - [x] **Проверка (pass/fail):** CI зелёный; при удалении раздела `Contracts` или пустых dev docs скрипт падает (exit 1).
...
## Тестирование
- [x] smoke: единая команда (или набор команд), который прогоняет lint/test/build для Rust и Browser (указана в docs)
- [x] security smoke: локальный запуск `gitleaks` (команда указана в docs)
