...
- [x] CI job `agent-spool-chaos-smoke (runtime chaos smoke)` существует и зелёный (выполняет `scripts/tests/agent_spool_chaos_runtime.sh`: kill -9 + full + disk_full + corruption)
