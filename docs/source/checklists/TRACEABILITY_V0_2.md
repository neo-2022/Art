...
- Корневые документы проекта и их обязательная синхронизация задаются в `formats/root_decision_tree_dependencies.yaml`; ствол (`дефектовочная ведомость -> MASTER`) и обзорные документы обязаны обновляться синхронно с изменением корня.
