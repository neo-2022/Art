...
- Корневые документы проекта и их обязательная синхронизация задаются в `formats/root_decision_tree_dependencies.yaml`; `MASTER`, дефектовочная ведомость и обзорные документы обязаны обновляться синхронно с изменением корня.
