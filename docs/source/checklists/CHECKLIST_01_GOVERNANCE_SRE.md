...
1. [ ] **Сделать:** Создать RACI матрицу ролей проекта: Owner, Maintainer, Reviewer, On-call, Security, Release.
   - [ ] причина → фикс → критерий готовности: в текущей матрице нет явного перечня ответственных по ролям; добавить владельцев ролей и PASS после проверки содержимого.  
   **Проверка (pass/fail):** существует файл `docs/governance/roles_raci.md`, содержит таблицу RACI и ответственных по ролям.

2. [ ] **Сделать:** Описать on-call процесс: расписание дежурств, канал оповещений, порядок передачи смены, контакты для эскалации.
   - [ ] причина → фикс → критерий готовности: в документе нет конкретных контактов эскалации; добавить контакты/каналы и PASS после повторной проверки.  
...
3. [ ] **Сделать:** Ввести таксономию severity SEV0–SEV3 с формальными критериями и SLA реакции.  
...
4. [ ] **Сделать:** Описать incident lifecycle: detect → triage → mitigate → resolve → postmortem → follow-ups.
   - [ ] причина → фикс → критерий готовности: lifecycle описан без явных правил открытия/закрытия инцидента; дополнить и проверить по тексту документа.  
...
5. [ ] **Сделать:** Зафиксировать правила регистрации и эскалации `observability_gap.*`:
   - [ ] причина → фикс → критерий готовности: раздел escalation слишком общий и не фиксирует auto-incident для ingest/spool/storage с min SEV1; привести к точным правилам шага и перепроверить.
...
6. [ ] **Сделать:** Создать реестр `observability_gap.*` (единый список допустимых `observability_gap.*` событий) с обязательными полями для каждой записи:
...
8. [ ] **Сделать:** Зафиксировать SLO/SLI с численными целями и окном измерения для минимум следующих SLI:
   - `ingest_success_rate`
...
9. [ ] **Сделать:** Привязать нарушение SLO к созданию инцидента, severity и действию (runbook).  
...
10. [ ] **Сделать:** Ввести error budget политику: поведение при исчерпании бюджета ошибок.  
...
12. [ ] **Сделать:** Описать change management политику: только PR в main, обязательный review, запрет прямых коммитов в main, ссылки на чек-лист этапа, требования к описанию изменений.
   - [ ] причина → фикс → критерий готовности: policy не фиксирует минимум 1 reviewer из CODEOWNERS; дополнить и проверить по документу.  
...
14. [ ] **Сделать:** Внедрить issue templates: incident и bug.  
    **Проверка (pass/fail):** существуют файлы:
...
17. [ ] **Сделать:** Описать policy MCP режимов: enable флаг, режимы `read_only`, `limited_actions`, `full_admin`; запреты действий в `read_only`.
   - [ ] причина → фикс → критерий готовности: в `limited_actions` нет явного перечня разрешённых action; добавить allowlist и проверить содержимое.  
...
18. [ ] **Сделать:** Описать политику аудита действий (Actions/Audit):
   - [ ] причина → фикс → критерий готовности: поле `actor` не раскрыто как роль/идентификатор, требуется доведение до точного контракта шага; после правки повторить проверку.
...
19. [ ] **Сделать:** Описать policy обработки уязвимостей: каналы репорта, triage, сроки реакции по severity, фиксация в issues.
   - [ ] причина → фикс → критерий готовности: текущая policy не фиксирует явное отражение кейсов в issues/трекере в терминах шага; дополнить и перепроверить.  
    **Проверка (pass/fail):** существуют файлы:
    - `SECURITY.md`
    - `docs/governance/vulnerability_process.md`

20. [ ] **Сделать:** Описать evidence policy для проверок чек-листов и для инцидентов.  
...
21. [ ] **Сделать:** Провести tabletop exercise по двум сценариям и зафиксировать результат:
...
22. [ ] **Сделать:** Добавить CI gate, который валидирует Stage 01:
