A) Полный запрет опциональности:
# CHECKLIST 24 — Release/Upgrade/Regression
Файл: CHECKLIST_24_RELEASE_UPGRADE_REGRESSION.md  
Последняя актуализация: 2026-03-04  
Дата последней проверки: 2026-03-05  
Триггер пересмотра: изменение релизной политики; изменение signing; изменение матрицы совместимости; изменение downgrade/миграций
Master checklist: docs/source/checklists/CHECKLIST_00_MASTER_ART_REGART.md

## Цель
Релиз однозначен и проверяем: manual release процесс; upgrade N→N+1 и downgrade N→N-1 с сохранением данных обязателен; cosign signing+verify для Docker образов обязателен; RELEASE_CHECKLIST обязателен перед тегом; отказ verify порождает `observability_gap.release_signing_failed`.

## Границы
Release/upgrade/regression процесс и тесты. Не включает разработку функционала Core/Agent.

## Зависимости
- CHECKLIST 23 — Ops/Deploy/Runbooks/DR
- CHECKLIST 04 — Secure SDLC + Supply-chain
- docs/source/trust_boundary_hardening_v0_2.md
- docs/source/browser_surface_hardening_v0_2.md
- docs/source/ingress_perimeter_protection_v0_2.md
- docs/source/regart_adversarial_integration_harness_v0_2.md
- docs/source/storage_pressure_protection_v0_2.md
- docs/source/startup_config_safety_validator_v0_2.md
- docs/source/queue_integrity_protection_v0_2.md
- docs/source/guard_self_observability_v0_2.md

## Шаги (строго линейно)

- [ ] **1. Сделать:** Зафиксировать manual release процесс (SemVer + CHANGELOG) и запретить “релиз локально”.
  - [ ] Релиз создаётся только через CI workflow (локальные релизы запрещены)
  - [ ] Версионирование фиксировано: SemVer
  - [ ] CHANGELOG обновляется в PR до тега
  - [ ] Тегирование выполняется только после выполнения RELEASE_CHECKLIST (шаг 4)
  - [ ] **Проверка (pass/fail):** `docs/release/release_process.md` существует и содержит все пункты выше в явном виде.

- [ ] **2. Сделать:** Ввести upgrade/downgrade regression suite (автоматизировано): N→N+1 и N→N-1 с сохранением данных.
  - [ ] Suite имеет фиксированное имя запуска: `make test-upgrade-downgrade`
  - [ ] Upgrade сценарий N→N+1:
    - [ ] поднимаем N, ingest 100 событий
    - [ ] создаём минимум 2 инцидента (по правилам/тестовым триггерам)
    - [ ] обновляем до N+1 (миграции выполняются по Stage 23)
    - [ ] проверяем: инциденты читаются через API, snapshot/stream работают
  - [ ] Downgrade сценарий N→N-1:
    - [ ] поднимаем N, ingest 100 событий, создаём минимум 2 инцидента
    - [ ] выполняем downgrade до N-1
    - [ ] проверяем: инциденты читаются через API, snapshot/stream работают
  - [ ] Критерий “сохранение данных” фиксирован:
    - [ ] после upgrade/downgrade количество инцидентов равно исходному
    - [ ] `ack.upto_seq` монотонен на прогоне
  - [ ] **Проверка (pass/fail):** suite запускается локально и в CI и зелёная.

- [ ] **3. Сделать:** cosign: подписывать Docker образы и проверять подпись в CI (sign + verify обязательны).
  - [ ] Подписываются Docker образы фиксированного набора:
    - [ ] `art-core` image
    - [ ] `art-agent` image
  - [ ] Инструмент: `cosign` (sigstore)
  - [ ] Режим: keyless OIDC (из Stage 04)
  - [ ] В CI есть workflow job `image-signing-verify`:
    - [ ] выполняет build images
    - [ ] выполняет cosign sign
    - [ ] выполняет cosign verify
    - [ ] падает при любой ошибке verify
  - [ ] **Проверка (pass/fail):** job `image-signing-verify` зелёный на релизном workflow и на PR (где применимо).

- [ ] **4. Сделать:** RELEASE_CHECKLIST обязателен перед тегом (enforce).
  - [ ] существует файл `RELEASE_CHECKLIST.md`
  - [ ] `RELEASE_CHECKLIST.md` содержит минимум:
    - [ ] версия (SemVer)
    - [ ] ссылка на CHANGELOG entry
    - [ ] ссылка на успешный прогон `make test-upgrade-downgrade`
    - [ ] ссылка на успешный `image-signing-verify`
    - [ ] ссылка на DR/backup требования (Stage 23)
  - [ ] В release workflow enforce правило: релиз/тегирование запрещено, если `RELEASE_CHECKLIST.md` не обновлён в текущем PR/коммите релиза
  - [ ] **Проверка (pass/fail):** induced test/проверка: попытка релиза без обновления `RELEASE_CHECKLIST.md` падает в CI.

- [ ] **5. Сделать:** `observability_gap.release_signing_failed` при провале signing verify.
  - [ ] Событие генерируется при любой ошибке verify подписи образов в release pipeline
  - [ ] Событие попадает в snapshot/stream (через startup backlog, если релизный пайплайн вне Core; фиксированная доставка: запись в локальный буфер и публикация при следующем старте Core)
  - [ ] evidence_min:
    - [ ] image_name
    - [ ] image_digest
    - [ ] cosign_error
    - [ ] trace_id
  - [ ] Событие зарегистрировано в `docs/governance/observability_gap_registry.md` с:
    - [ ] `incident_rule=create_incident_min_sev1`
    - [ ] `action_ref=docs/runbooks/release_signing_failed.md`
  - [ ] **Проверка (pass/fail):** induced test ломает verify (например, подменяет digest) и проверяет:
    - [ ] падение verify шага в CI
    - [ ] наличие `observability_gap.release_signing_failed` в snapshot/stream после следующего старта Core.

- [ ] **6. Сделать:** Зафиксировать release evidence narrative и customer-facing артефакты релиза.
  - [ ] Каждый релиз обновляет `docs/portal/DELIVERY_EVIDENCE.md`.
  - [ ] Каждый релиз обновляет compatibility matrix и release decision record.
  - [ ] Для релиза существует заполняемый `GO/NO-GO` record, связанный с `RELEASE_CHECKLIST.md`.
  - [ ] Для internet-exposed профиля release evidence явно подтверждает наличие ingress/perimeter shield baseline.
  - [ ] **Проверка (pass/fail):** release docs gate подтверждает наличие связки `RELEASE_CHECKLIST -> GO/NO-GO -> DELIVERY_EVIDENCE -> compat matrix`.

- [ ] **7. Сделать:** Ввести release-blocker для internet-exposed профиля без anti-DDoS / perimeter baseline.
  - [ ] internet-exposed rollout запрещён без front-door / edge / ingress shield policy
  - [ ] `RELEASE_CHECKLIST.md` и `GO/NO-GO` record обязаны ссылаться на hostile ingress proof или validate-path shield baseline
  - [ ] попытка релиза internet-exposed профиля без этого evidence валится в CI
  - [ ] **Проверка (pass/fail):** induced release-check fail подтверждает, что internet-exposed rollout без shield baseline блокируется.

- [ ] **8. Сделать:** Ввести release-blocker для недоказанной trust boundary и browser surface hardening baseline.
  - [ ] internet-exposed и privileged release profile запрещён без trusted actor context proof.
  - [ ] internet-exposed browser surface запрещён без CSP/frame/integrity baseline и negative-path evidence.
  - [ ] `GO/NO-GO` и release evidence обязаны ссылаться на trust boundary и browser surface proof.
  - [ ] **Проверка (pass/fail):** induced release-check fail подтверждает, что отсутствие trust boundary/browser surface proof блокирует релиз.

- [ ] **9. Сделать:** Ввести release-blocker для partner-exposed и REGART-интеграций без pinned external adversarial harness proof.
  - [ ] release evidence обязано ссылаться на pinned source manifest external harness.
  - [ ] partner-exposed / REGART integration не считаются доказанными без suite `smoke / hostile-bridge / replay / long-chain / actions-audit` там, где применимо.
  - [ ] floating checkout или unpinned external source запрещены как release evidence.
  - [ ] **Проверка (pass/fail):** induced release-check fail подтверждает, что отсутствие pinned harness proof блокирует релиз.

- [ ] **10. Сделать:** Ввести release-blocker для недоказанной защиты от `storage pressure` и `disk exhaustion`.
  - [ ] production release запрещён без evidence, что high/critical watermark policy materialized и проверена hostile smoke.
  - [ ] `RELEASE_CHECKLIST.md` и `GO/NO-GO` record обязаны ссылаться на proof `storage_pressure_high` / `storage_disk_full`.
  - [ ] **Проверка (pass/fail):** induced release-check fail подтверждает, что отсутствие storage pressure proof блокирует релиз.

- [ ] **11. Сделать:** Ввести release-blocker для отсутствия startup fail-closed baseline.
  - [ ] privileged/internet-exposed rollout запрещён, если unsafe config не блокируется на старте.
  - [ ] release evidence обязано ссылаться на `unsafe_startup_config_refused` proof.
  - [ ] **Проверка (pass/fail):** induced release-check fail подтверждает, что отсутствие fail-closed proof блокирует релиз.

- [ ] **12. Сделать:** Ввести release-blocker для отсутствия queue integrity / anti-loop proof.
  - [ ] release запрещён без hostile proof duplicate flood / replay loop protection.
  - [ ] release evidence обязано ссылаться на `queue_integrity_violation` proof.
  - [ ] **Проверка (pass/fail):** induced release-check fail подтверждает, что отсутствие queue integrity proof блокирует релиз.

- [ ] **13. Сделать:** Ввести release-blocker для отсутствия guard self-observability proof.
  - [ ] release запрещён, если критические guards не имеют startup self-test, heartbeat и failure event.
  - [ ] release evidence обязано ссылаться на `guard_self_test_failed` negative-path proof.
  - [ ] **Проверка (pass/fail):** induced release-check fail подтверждает, что отсутствие guard self-observability proof блокирует релиз.

## Документация (RU)
- [ ] docs/release/release_process.md
- [ ] docs/release/versioning.md
- [ ] docs/release/compat_matrix.md
- [ ] docs/release/downgrade.md
- [ ] docs/runbooks/release_signing_failed.md
- [ ] RELEASE_CHECKLIST.md
- [ ] CHANGELOG.md
- [ ] docs/portal/DELIVERY_EVIDENCE.md
- [ ] docs/governance/release_decisions/latest_go_no_go.md
- [ ] docs/source/ingress_perimeter_protection_v0_2.md
- [ ] docs/source/trust_boundary_hardening_v0_2.md
- [ ] docs/source/browser_surface_hardening_v0_2.md
- [ ] docs/source/storage_pressure_protection_v0_2.md
- [ ] docs/source/startup_config_safety_validator_v0_2.md
- [ ] docs/source/queue_integrity_protection_v0_2.md
- [ ] docs/source/guard_self_observability_v0_2.md
- [ ] docs/runbooks/storage_pressure_high.md
- [ ] docs/runbooks/unsafe_startup_config_refused.md
- [ ] docs/runbooks/queue_integrity_violation.md
- [ ] docs/runbooks/guard_self_test_failed.md

## Тестирование
- [ ] integration: `make test-upgrade-downgrade` (runtime API suite, шаг 2)
- [ ] security: `image-signing-verify` (шаг 3)
- [ ] induced: signing verify fail → `observability_gap.release_signing_failed` (шаг 5)
- [ ] docs/release evidence chain: `RELEASE_CHECKLIST -> GO/NO-GO -> DELIVERY_EVIDENCE -> compat matrix` непротиворечива.
- [ ] release blocker: internet-exposed profile without ingress shield baseline -> FAIL.
- [ ] release blocker: trust boundary или browser surface baseline отсутствует -> FAIL.
- [ ] release blocker: partner-exposed / REGART profile без pinned external adversarial harness proof -> FAIL.
- [ ] release blocker: storage pressure proof отсутствует -> FAIL.
- [ ] release blocker: startup fail-closed proof отсутствует -> FAIL.
- [ ] release blocker: queue integrity proof отсутствует -> FAIL.
- [ ] release blocker: guard self-observability proof отсутствует -> FAIL.

## CI gate
- [ ] CI job `test-upgrade-downgrade` существует и зелёный
- [ ] CI job `image-signing-verify` существует и зелёный
- [ ] CI job `stage24-docs-gate` существует и запускается на PR в main
- [ ] `stage24-docs-gate` запускает `scripts/ci/check_release_stage24_docs.sh`, который:
  - [ ] проверяет существование файлов из раздела “Документация (RU)”
  - [ ] проверяет минимальный контент (grep):
    - [ ] `docs/release/release_process.md` содержит `manual` и `CI` и `SemVer`
    - [ ] `docs/release/downgrade.md` содержит `N-1` и `инциденты читаются`
    - [ ] `RELEASE_CHECKLIST.md` содержит `test-upgrade-downgrade` и `image-signing-verify`
    - [ ] `docs/runbooks/release_signing_failed.md` содержит `mitigations` и `verification`
    - [ ] `docs/governance/observability_gap_registry.md` содержит `release_signing_failed`
    - [ ] `RELEASE_CHECKLIST.md` или `docs/governance/release_decisions/latest_go_no_go.md` содержит `ingress/perimeter`
    - [ ] `RELEASE_CHECKLIST.md` или `docs/governance/release_decisions/latest_go_no_go.md` содержит `pinned harness`
    - [ ] `RELEASE_CHECKLIST.md` или `docs/governance/release_decisions/latest_go_no_go.md` содержит `storage pressure`
    - [ ] `RELEASE_CHECKLIST.md` или `docs/governance/release_decisions/latest_go_no_go.md` содержит `unsafe startup config`
    - [ ] `RELEASE_CHECKLIST.md` или `docs/governance/release_decisions/latest_go_no_go.md` содержит `queue integrity`
    - [ ] `RELEASE_CHECKLIST.md` или `docs/governance/release_decisions/latest_go_no_go.md` содержит `guard self-test`
  - [ ] exit 1 при нарушении любой проверки

## DoD
- [ ] Manual release процесс задокументирован и enforce’ится CI.
- [ ] Upgrade/downgrade suite автоматизирована и зелёная в CI.
- [ ] cosign sign+verify для Docker образов обязательны и зелёные.
- [ ] RELEASE_CHECKLIST обязателен и реально блокирует релиз без заполнения.
- [ ] `observability_gap.release_signing_failed` реализован, зарегистрирован и покрыт induced test.
- [ ] Release evidence narrative обновляется как часть релиза и делает прогресс/совместимость видимыми вне CI логов.
- [ ] Internet-exposed release profile блокируется без hostile ingress/perimeter baseline.
- [ ] Trust boundary и browser surface hardening являются явными release blockers для internet-exposed и privileged rollout.
- [ ] Pinned external adversarial harness является явным release blocker для partner-exposed и REGART-bridge rollout.
- [ ] Storage pressure / disk exhaustion proof является явным release blocker.
- [ ] Startup fail-closed proof является явным release blocker.
- [ ] Queue integrity / anti-loop proof является явным release blocker.
- [ ] Guard self-observability proof является явным release blocker.
- [ ] CI gate Stage 24 зелёный.

## Финальный блокирующий чекбокс (единое жёсткое правило)
- [ ] Этап/лист закрывается только после фактического прохождения всех пунктов этого листа: каждый пункт имеет PASS-проверку и подтверждённый артефакт (тест/лог/команда/файл/CI), и только после этого ставится финальная отметка закрытия.
