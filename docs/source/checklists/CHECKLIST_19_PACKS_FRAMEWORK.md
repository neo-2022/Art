...
- [x] runtime: `scripts/tests/pack_install_runtime.sh` проверяет install из реального `packs/regart` layout (`manifest.yaml` + `payload/` + `signatures/manifest.sha256`)

## CI gate
- [x] CI job `packs-tests` существует и запускается на PR в main; job зелёный
- [x] CI job `packs-runtime-install` существует и запускается на PR в main; job зелёный
...
- [x] `stage19-docs-gate` запускает `scripts/ci/check_packs_stage19_docs.sh`, который:
  ...
  - [x] проверяет существование runtime harness `scripts/tests/pack_install_runtime.sh`
