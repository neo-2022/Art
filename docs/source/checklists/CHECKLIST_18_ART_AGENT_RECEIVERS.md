...
- [ ] chaos runtime matrix: `scripts/tests/agent_receivers_chaos_runtime.sh` (permission denied/spawn failed/parse failed/multiline oversize/unsupported kind/redaction)
...
- [ ] `stage18-docs-gate` запускает `scripts/ci/check_agent_receivers_stage18_docs.sh`, который:
  ...
  - [ ] проверяет минимальный контент (grep):
    ...
    - [ ] `docs/agent/receivers_chaos.md` содержит `agent_receivers_chaos_runtime.sh` и `receiver_process_spawn_failed`
