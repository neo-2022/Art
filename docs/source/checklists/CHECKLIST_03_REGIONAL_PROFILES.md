...
- [ ] **3. Сделать:** Описать смену профиля как единственную допустимую операцию переключения (процедура фиксирована).
  - [ ] остановить ingest
  - [ ] остановить Core
  - [ ] применить новую конфигурацию (включая `profile_id`)
  - [ ] стартовать Core
  - [ ] выполнить проверку guardrails профиля (шаг 6)
  - [ ] стартовать ingest
  - [ ] **Проверка (pass/fail):** `docs/compliance/profiles.md` содержит раздел `profile switch procedure` с ровно этими шагами в указанном порядке.

- [ ] **4. Сделать:** Описать матрицу совместимости переходов профилей и требования к миграции/валидации данных.
  - [ ] список допустимых переходов `from → to`
  - [ ] для каждого допустимого перехода: обязательные действия (migrate/validate/purge/reindex) и критерий “готово”
  - [ ] для каждого недопустимого перехода: явный запрет и причина
  - [ ] **Проверка (pass/fail):** `docs/compliance/profiles.md` содержит раздел `migration/validation` и таблицу `transition matrix` со всеми профилями и правилами для каждого `from → to`.
